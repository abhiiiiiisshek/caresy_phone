...
export function CapyMascot({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(bob, { toValue: -8, duration: 1100, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
        Animated.timing(bob, { toValue: 0, duration: 1100, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(earL, { toValue: 1, duration: 1800, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
        Animated.timing(earL, { toValue: 0, duration: 1800, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
      ])
    ).start();
    Animated.loop(
      Animated.sequence([
        Animated.delay(900),
        Animated.timing(earR, { toValue: 1, duration: 2000, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
        Animated.timing(earR, { toValue: 0, duration: 2000, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
      ])
    ).start();
  ...
  useEffect(() => {
    const loop = Animated.loop(
      Animated.sequence([
        ...
        Animated.timing(y, { toValue: -6, duration: 1400 + delay * 2, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
        Animated.timing(y, { toValue: 0, duration: 1400 + delay * 2, easing: Easing.inOut(Easing.sine), useNativeDriver: true }),
      ])
    );
    loop.start();
    return () => loop.stop();
