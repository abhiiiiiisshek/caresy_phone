...
| 51 | `51_ADMIN_DEVICE_PUSH.sql` | `admin_push_user_ids()` (SECURITY DEFINER, `service_role` only) so `api/cron/send-push` can fan ADMIN-role rows — which carry no `recipient_user_id` — out to every admin device in `push_tokens`; `enqueue_upcoming_unstaffed_bookings(p_lead_minutes)` enqueues one deduped ADMIN row per scheduled visit that is still unstaffed inside its lead window (new event `BOOKING_UPCOMING_UNSTAFFED`) | ⬜ |
